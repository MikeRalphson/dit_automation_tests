...
#DN-114 - Pre-roll and Post-roll ads
Scenario: Request a video with adverts
...
Scenario: Request a video with no adverts
	Given a Lithium video is available
...
@wip
@ipOutSideUK
Scenario: Request a geoblocked video from outside the UK
...
Scenario: Request a geoblocked video from inside the UK
	Given a Lithium video is available with IsGeoblocked set to true
	When I request a Lithium playlist for the video from inside the UK
	Then I should receive a valid playlist

#DN-116 Encryption
Scenario: Request an playlist with HLS stream type
	Given a Lithium video is available 
	When I request a Lithium playlist for HLS
	Then I should receive a valid playlist
	And the playlist should be for HLS
	
Scenario: Request an playlist with no stream type
	Given a Lithium video is available
