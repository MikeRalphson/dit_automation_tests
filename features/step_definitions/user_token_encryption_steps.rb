...
Given /^a request for archive content without a UserToken$/ do
  @prodid = @vodcrid_helpers.get_prodid_from_config("DotCom")
end
...
Given /^a request for archive content containing a malformed JOSN UserToken$/ do
  @prodid = @vodcrid_helpers.get_prodid_from_config("DotCom")
  @encrypted = generate_invalid_encrypted_usertoken(@prodid, @user_id, DateTime.now)
end

Given /^a request for archive content containing an expired UserToken$/ do
  @prodid = @vodcrid_helpers.get_prodid_from_config("DotCom")
  @encrypted = generate_encrypted_usertoken(@prodid, @user_id, DateTime.iso8601('2011-10-19T12:16:21+00:00'))
end

Given /^a request for archive content containing a UserToken in the future$/ do
  @prodid = @vodcrid_helpers.get_prodid_from_config("DotCom")
  @encrypted = generate_encrypted_usertoken(@prodid, @user_id, DateTime.iso8601('2013-10-19T12:16:21+00:00'))
end
...
When /^the user makes a initial (.*) playlist request for the catchup content$/ do |platform|
  @playlist_client = @mercury_playlist.create_client
  @prodid = @vodcrid_helpers.get_prodid_from_config("CatchUp")
  begin
    @response = @mercury_playlist.encrypted_playlist_request(@playlist_client, @prodid, @encrypted, platform)
  rescue Savon::SOAP::Fault => error
    @playlist_error = error
...
When /^the user makes a subsequent (.*) playlist request for the catchup content$/ do |platform|
  @prodid = @vodcrid_helpers.get_prodid_from_config("CatchUp")
  @encrypted = generate_encrypted_usertoken(@prodid,"1237847", DateTime.now)
  @playlist_client = @mercury_playlist.create_client
  begin
    @response = @mercury_playlist.encrypted_playlist_request(@playlist_client, @prodid, @encrypted, platform)
  rescue Savon::SOAP::Fault => error
